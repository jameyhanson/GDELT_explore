...
-- weekly moving averages for the 7 days before each Tuesday
...
grp_week_host = GROUP w_usa_actors BY (ew_date_tue, host, tld);

host_records_by_week = FOREACH grp_week_host GENERATE
    FLATTEN(group) AS (ew_date_tue, host, tld),
    COUNT(w_usa_actors) AS num_records;
    
grp_host_records_by_week = GROUP host_records_by_week BY (ew_date_tue);

host_records_by_week_ntiles = FOREACH grp_host_records_by_week GENERATE
    FLATTEN(group) AS (ew_date_tue),
    Quantile(host_records_by_week.num_records) AS num_records_ntile;
    
host_records_and_ntiles_by_week = JOIN
    host_records_by_week BY ew_date_tue,
    host_records_by_week_ntiles BY ew_date_tue;
...
hosts_that_report_alot_on_USA = FOREACH hosts_that_report_alot_on_USA GENERATE
    host_records_by_week::ew_date_tue AS ew_date_tue,
...
AvgTone_about_USA_by_week = GROUP w_usa_actors BY ew_date_tue;
...
w_usa_AvgTone_and_ntiles_by_week = JOIN
    AvgTone_about_USA_by_week_ntiles BY ew_date_tue,
    w_usa_actors BY ew_date_tue;
...
very_negative_tone_about_USA_by_week = GROUP very_negative_tone_about_USA BY (
    w_usa_actors::ew_date_tue,
    w_usa_actors::host,
    w_usa_actors::tld);
    
host_count_of_very_negative_by_week = FOREACH very_negative_tone_about_USA_by_week GENERATE
    FLATTEN(group) AS (w_usa_actors::ew_date_tue, w_usa_actors::host, w_usa_actors::tld),
    COUNT(very_negative_tone_about_USA) AS num_very_negative_records;
   
join_host_counts_by_week = JOIN
    hosts_that_report_alot_on_USA BY (ew_date_tue, host),
    host_count_of_very_negative_by_week BY (ew_date_tue, host);
    
fraction_of_very_negative_by_week = FOREACH join_host_counts_by_week GENERATE
    hosts_that_report_alot_on_USA::ew_date_tue AS ew_date_tue,
...
STORE hosts_with_lots_of_very_negative_by_week INTO '$RESULTS_BASE_DIR/hosts_with_lots_of_very_negative/02Tue_results';
